...
# Переменная-флаг: сообщает форме, что Python успешно сохранил данные и можно очистить поля
...
if not os.path.exists('production.db'):
    ...
    # Превращаем триггер очистки в JS-переменную (true/false)
    should_clear_js = "true" if st.session_state.clear_form_trigger else "false"
    
    # Сбрасываем триггер в Python, так как текущий рендеринг уже получит эту команду
    ...
    <script>
        ...
        // ЛОГИКА ОЧИСТКИ ПОСЛЕ СОХРАНЕНИЯ:
        // Если Python передал true, значит данные в сессии. Восстанавливаем форму пустой.
        // Если false, пытаемся восстановить то, что писал пользователь, чтобы текст не пропадал при ошибках базы данных.
        if ({should_clear_js}) {{
            localStorage.removeItem('draft_item');
            localStorage.removeItem('draft_ops');
            localStorage.removeItem('draft_serials');
            inpItem.value = '';
            inpOps.value = '';
            inpSerials.value = '';
        }} else {{
            inpItem.value = localStorage.getItem('draft_item') || '';
            inpOps.value = localStorage.getItem('draft_ops') || '';
            inpSerials.value = localStorage.getItem('draft_serials') || '';
        }}
        ...
        function sendData(e) {{
            ...
            // Запоминаем текущий ввод в кэш браузера, чтобы он не стерся во время перезагрузки страницы Streamlit
            localStorage.setItem('draft_item', inpItem.value);
            localStorage.setItem('draft_ops', inpOps.value);
            localStorage.setItem('draft_serials', inpSerials.value);
            
            submitBtn.textContent = "⏳ Расчёт и сохранение...";
            submitBtn.style.backgroundColor = "#ccc";
            submitBtn.disabled = true;

            // Находим скрытый текстовый мост Streamlit в родительском DOM и пишем строго туда.
            // Никакой мгновенной очистки полей в этот момент НЕ ДЕЛАЕМ! Поля остаются заполненными.
    ...
    # Создаем скрытый текстовый мост
    form_data_raw = st.text_input("Вспомогательный мост:", key="hidden_bridge", label_visibility="collapsed")
    
    # Отрисовываем HTML форму. Благодаря key= форма не будет мерцать и перезапускаться сама по себе
    components.html(html_form, height=330, key="html_form_component")
            ...
            if selected_name and ops_raw and serials_raw:
                ...
                if not ok: 
                    ...
                    if not found: 
                        ...
                        # ДАННЫЕ УСПЕШНО СОХРАНЕНЫ В PYTHON:
                        # 1. Зануляем скрытый мост, чтобы избежать повторных добавлений деталей
                        st.session_state.hidden_bridge = ""
                        # 2. Включаем триггер очистки формы: при следующем рендере JS увидит его и сотрет инпуты
                        st.session_state.clear_form_trigger = True
                        
                        st.success("Успешно добавлено!")
                        st.rerun() # Перезапускаем страницу: верхняя сумма обновится, а форма очистится
