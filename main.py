...
# Храним временную JSON строку, чтобы избежать дублирования циклов
...
if not os.path.exists('production.db'):
    ...
    if os.path.exists(html_path):
        ...
        # Если в прошлый раз успешно добавили деталь, передаем iframe параметр очистки полей
        if st.headers.get("x-clear-form") or st.query_params.get("saved") == "1":
            html_template = html_template.replace("window.location.hash = '';", "window.location.hash = '';")
            
        html_form = html_template.replace('__ITEMS_PLACEHOLDER__', js_items)
        
        # Выводим HTML через стандартный стабильный метод. Ошибки TypeError больше не будет!
        components.html(html_form, height=330)
    else:
        st.error("Файл 'autocomplete.html' не найден рядом со скриптом!")
    # ОБРАБОТКА ДАННЫХ ИЗ НАДЁЖНОГО КАНАЛА QUERY PARAMETERS
    q_item = st.query_params.get("item", "").strip()
    q_ops = st.query_params.get("ops", "").strip()
    q_serials = st.query_params.get("serials", "").strip()

    # Если в URL пришли параметры от успешной вставки
    if q_item and q_ops and q_serials:
        try:
            # Очищаем параметры браузера, чтобы не зацикливать добавление деталей
            ...
            if not ok: 
                ...
                with sqlite3.connect('production.db') as conn:
                    ...
                    for op in ops:
                        ...
                        res = cursor.fetchone()
                        if res: 
                            # ТОЧНЫЕ ИНДЕКСЫ СТРОК SQL: res[0]=чертеж, res[1]=описание, res[2]=цена
                            found.append({
                                'op_num': op, 
                    ...
                    if not found: 
                        ...
                        # Сообщаем форме, что сохранение прошло успешно, добавив временный query параметр
    ...
    # Блок очистки параметра saved при следующем рендере
